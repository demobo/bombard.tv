...
function OnLoad() {
	...
	if (curVideo) {
		videoSearch
				.setResultOrder(google.search.Search.ORDER_BY_ASCENDING_DATE);
	} else {
	...
	if (curVideo) {
		var kw = curVideo.tags.split(',', 3).toString();
		searchControl.execute(kw);
	} else {
...
OnSearchComplete = function(sc, searcher) {
	...
	$(".gsc-resultsbox-visible").hide();
	for ( var i = 0; i < searcher.results.length; i++) {
		// console.log(searcher.results[i]);
		var vid = searcher.results[i].tbUrl.split('/')[4];
		var videoresult = $('<div class="videoresult"><a onclick="javascript:loadVideo(\''
				+ vid
				+ '\')"><img src="'
				+ searcher.results[i].tbUrl
				+ '"></a></div>');
		$("#Videoresults").append(videoresult);
		var tipText = searcher.results[i].title;
		if (searcher.results[i].viewCount)
			tipText += "<br>" + "View Count: "
					+ addCommas(searcher.results[i].viewCount)
		videoresult.qtip( {
			content : tipText,
			show : {
				ready : false
			...
			style : {
				classes : 'ui-tooltip-blue ui-tooltip-youtube',
				width : 300,
				height : 100
			}
		});
...
function onPlayerStateChange(newState) {
	var realVideoID = ytplayer.getVideoUrl().split('=')[1].split('&')[0];
	if (videoID!=realVideoID && realVideoID != 'player_embedded') {
		ytplayer.stopVideo();
		loadVideo(realVideoID);
	}
}
